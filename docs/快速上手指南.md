...
强化学习将世界分为两部分，智能体(Agent)和环境(Environment)。智能体从环境中感知到当前时刻的状态$S_t$和奖励$R_t$，之后根据感知信息进行决策并采取动作$A_t$，动作被执行后影响环境进入下一个状态。智能体继续获得下一时刻的状态$S_{t+1}$和奖励$R_{t+1}$。智能体的目的是获得的奖励尽可能多。

+ **马尔可夫决策过程**

马尔可夫决策过程可以用五元组表示$<S,A,P,R,\gamma>$，其中，$S$为状态集合，$A$为动作集合，$P$为状态转移概率，$R$为奖励函数，$\gamma$为折扣因子。
...
+ Behavior Parameters
  + `Behavior Name`：行为名称
  + `Vector Observation->Space Size`：向量观测大小，与训练脚本中状态空间设置有关(见4.2.3 收集观测函数，1+1+1+18=21)；二者不一致，编译器会给出警告
...
public override void CollectObservations(VectorSensor sensor)
{
    ...
    sensor.AddObservation(localVelocity.x);  //将无人艇x方向速度加入状态空间，1维向量
    sensor.AddObservation(localVelocity.z);  //将无人艇z方向速度加入状态空间，1维向量
...
public void MoveAgent(ActionSegment<int> act)
{
    ...
    rBody.AddForce(dirToGo * moveSpeed, ForceMode.VelocityChange);	//前进

    if (rBody.velocity.sqrMagnitude > 4f) //防止速度过快
    {
        rBody.velocity *= 0.6f;
    }
}
...
与国赛训练场景基本一致，区别在于增加了5个固定的航路点，其余部分与前述场景框架一致。

到达某个航路点后，其颜色将从红色变为绿色，同时目标点切换至下一个航路点，直至到达终点
...
public override void OnActionReceived(ActionBuffers actionBuffers)
{
    ...
    if (point1 == false && Vector3.Distance(Target.transform.localPosition, transform.localPosition) < 1.5f) 
        //判断是否首先到达第一个航路点，如果是则修改第一航路点的颜色，同时将目标点设置为第二航路点
    {
        Waypoint1.GetComponent<Renderer>().material.color = new Color(0, 1.0f, 0);
        point1 = true;
        Target.transform.localPosition = Waypoint2.transform.localPosition;
    }
    if (point1 == true && point2 == false && Vector3.Distance(Target.transform.localPosition, transform.localPosition) < 1.5f)
        //判断是否在到达第一个航路点的情况下到达了第二航路点，如果是则修改第二航路点的颜色，同时将目标点设置为第三航路点
    {
        Waypoint2.GetComponent<Renderer>().material.color = new Color(0, 1.0f, 0);
        point2 = true;
        Target.transform.localPosition = Waypoint3.transform.localPosition;
    }
    if (point2 == true && point3 == false && Vector3.Distance(Target.transform.localPosition, transform.localPosition) < 1.5f)
        //判断是否在到达第二航路点的情况下到达了第三航路点，如果是则修改第三航路点的颜色，同时将目标点设置为第四航路点
    {
        Waypoint3.GetComponent<Renderer>().material.color = new Color(0, 1.0f, 0);
        point3 = true;
        Target.transform.localPosition = Waypoint4.transform.localPosition;
    }
    if (point3 == true && point4 == false && Vector3.Distance(Target.transform.localPosition, transform.localPosition) < 1.5f)
        //判断是否在到达第三航路点的情况下到达了第四航路点，如果是则修改第四航路点的颜色，同时将目标点设置为终点
    {
        Waypoint4.GetComponent<Renderer>().material.color = new Color(0, 1.0f, 0);
        point4 = true;
        Target.transform.localPosition = Starting_point.transform.localPosition;
    }
    if (point4 == true && Vector3.Distance(Target.transform.localPosition, transform.localPosition) < 1.5f)
        //判断是否在到达第四航路点的情况下到达了终点，如果是则结束该回合
    {
        Target.transform.localPosition = Starting_point.transform.localPosition;
        AddReward(MaxRew);
        EndEpisode();
...
训练无人艇，使其能够从固定起点出发，躲避障碍物，依次到达四个航路点并回到起点

### 4.3 Python脚本

Python脚本用于将Unity场景打包为Gym训练环境，并搭建用于训练的神经网络

参考脚本中使用了stable_baselines3库中的PPO算法网络(该方法适用于不太熟悉强化学习算法的选手)，详细介绍可查看官方文档：[Stable-Baselines3](https://stable-baselines3.readthedocs.io/en/master/)
...
UniytEnv = UE()
env = UnityToGymWrapper(UniytEnv,flatten_branched=True, allow_multiple_obs=False) #打包Unity环境
...
+ 根据任务设计状态空间和奖励函数，对应修改c#训练脚本中收集观测函数和动作反馈函数

+ 选择合适的强化学习算法，设计网络结构，优化算法参数
